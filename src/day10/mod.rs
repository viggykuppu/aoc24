...
pub fn one() {
    ...
    let map: Vec<Vec<_>> = input
        .lines()
        .map(|line| line.chars().map(|c| c.to_digit(10).unwrap_or(11)).collect())
        .collect();
    let total_trailheads: u32 = map
        .iter()
        .enumerate()
        .map(|(i, row)| {
            row.iter()
...
fn get_trailhead_score(
    ...
    for v in [(-1, 0), (0, -1), (1, 0), (0, 1)].iter() {
        ...
        if (0..(map.len() as isize)).contains(&new_location.0)
            && (0..(map[0].len() as isize)).contains(&new_location.1)
        {
