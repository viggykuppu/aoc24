...
pub fn one() {
    ...
    for _ in 0..100 {
        for robot in robots.iter_mut() {
            robot.position.0 =
                ((robot.position.0 + robot.velocity.0) % bounds.0 + bounds.0) % bounds.0;
            robot.position.1 =
                ((robot.position.1 + robot.velocity.1) % bounds.1 + bounds.1) % bounds.1;
        }
    }
    ...
    for robot in robots.iter() {
        ...
        let quad_score = (
            !(0..quad_bounds.0).contains(&robot.position.0) as i32,
            !(0..quad_bounds.1).contains(&robot.position.1) as i32,
        );
